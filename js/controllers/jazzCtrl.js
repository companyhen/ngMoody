app.controller('JazzCtrl', function($scope, soundService, $sce, $timeout, client_id, $filter) {
	var ctrl = this;

	ctrl.songs = [];
	ctrl.playerInfo = {};
	ctrl.isStreaming = true;
	ctrl.jazzSongs = soundService.then(function(result){
		ctrl.jazzSongs = result[6].data;
		ctrl.playerInfo.total = ctrl.jazzSongs.length;
		ctrl.playerInfo.song = 0;

		$filter('shuffle')(ctrl.jazzSongs);
		ctrl.url = $sce.trustAsResourceUrl(ctrl.jazzSongs[ctrl.playerInfo.song].stream_url + client_id);
		ctrl.currentSong = ctrl.jazzSongs[ctrl.playerInfo.song].title;
	});
