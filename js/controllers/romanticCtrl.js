app.controller('RomanticCtrl', function($scope, soundService, $sce, $timeout, client_id, $filter) {
	var ctrl = this;

	ctrl.songs = [];
	ctrl.playerInfo = {};
	ctrl.isStreaming = true;
	ctrl.romanticSongs = soundService.then(function(result){
		ctrl.romanticSongs = result[8].data;
		ctrl.playerInfo.total = ctrl.romanticSongs.length;
		ctrl.playerInfo.song = 0;

		$filter('shuffle')(ctrl.romanticSongs);
		ctrl.url = $sce.trustAsResourceUrl(ctrl.romanticSongs[ctrl.playerInfo.song].stream_url + client_id);
		ctrl.currentSong = ctrl.romanticSongs[ctrl.playerInfo.song].title;
	});
