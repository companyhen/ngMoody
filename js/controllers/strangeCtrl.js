app.controller('StrangeCtrl', function($scope, soundService, $sce, $timeout, client_id, $filter) {
	var ctrl = this;

	ctrl.songs = [];
	ctrl.playerInfo = {};
	ctrl.isStreaming = true;
	ctrl.strangeSongs = soundService.then(function(result){
		ctrl.strangeSongs = result[11].data;
		ctrl.playerInfo.total = ctrl.strangeSongs.length;
		ctrl.playerInfo.song = 0;

		$filter('shuffle')(ctrl.strangeSongs);
		ctrl.url = $sce.trustAsResourceUrl(ctrl.strangeSongs[ctrl.playerInfo.song].stream_url + client_id);
		ctrl.currentSong = ctrl.strangeSongs[ctrl.playerInfo.song].title;
	});
