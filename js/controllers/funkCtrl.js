app.controller('FunkCtrl', function($scope, soundService, $sce, $timeout, client_id, $filter) {
	var ctrl = this;

	ctrl.songs = [];
	ctrl.playerInfo = {};
	ctrl.isStreaming = true;
	ctrl.funkSongs = soundService.then(function(result){
		ctrl.funkSongs = result[7].data;
		ctrl.playerInfo.total = ctrl.funkSongs.length;
		ctrl.playerInfo.song = 0;

		$filter('shuffle')(ctrl.funkSongs);
		ctrl.url = $sce.trustAsResourceUrl(ctrl.funkSongs[ctrl.playerInfo.song].stream_url + client_id);
		ctrl.currentSong = ctrl.funkSongs[ctrl.playerInfo.song].title;
	});
